...
/** Parse a JSONL line, returning null for malformed content (sessions can contain partial writes). */
...
function readEntries(sessionFile: string): SessionEntry[] {
  return readLines(sessionFile)
    .map(tryParseEntry)
    .filter((entry): entry is SessionEntry => entry !== null);
}

/**
 * Count raw JSONL entries (non-blank lines) in a session file.
 * Use this as the `afterLine` offset for getNewEntries — it counts lines,
 * including malformed ones, so offsets stay stable.
 */
