...
export default function subagentsExtension(pi: ExtensionAPI) {
  ...
  async function launchSubagent(
    ...
    const launchResult = buildLaunchCommand({ cwd: effectiveCwd, env, argv });
    const launchCmd = typeof launchResult === "string" ? launchResult : launchResult.command;
    const batchFile = typeof launchResult === "object" ? launchResult.batchFile : undefined;
    sendPaste(surface, launchCmd);
  ...
  async function watchSubagent(
    ...
    const finish = (): void => {
      ...
      if (running.batchFile) {
        try { unlinkSync(running.batchFile); } catch { /* already deleted by the batch file itself */ }
      }
  ...
  if (shouldRegister("subagent_resume"))
    pi.registerTool({
      ...
      async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
        ...
        const resumeLaunch = buildLaunchCommand({ env, argv });
        const resumeCmd = typeof resumeLaunch === "string" ? resumeLaunch : resumeLaunch.command;
